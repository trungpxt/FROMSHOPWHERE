...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'update_info') {
        ...
        if (!$ten) { $error = 'Họ tên không được để trống.'; }
        else {
            db()->prepare("UPDATE users SET ho_ten=:t,so_dien_thoai=:s,dia_chi=:d WHERE id=:id")
               ->execute([':t'=>$ten,':s'=>$sdt,':d'=>$dia,':id'=>$user['id']]);
            $_SESSION['user_name'] = $ten;
            $msg = '✓ Đã cập nhật thông tin.';
            $u['ho_ten'] = $ten; $u['so_dien_thoai'] = $sdt; $u['dia_chi'] = $dia;
        }
    }
        ...
        <div class="info-box">
          ...
          <form method="POST">
            ...
            <div class="form-group">
              ...
              <input class="form-input" type="tel" name="so_dien_thoai" placeholder="0901 234 567" value="<?= e($u['so_dien_thoai']??'') ?>">
            </div>
            <div class="form-group">
