...
if ($coupon && isset($VALID_COUPONS[$coupon])) {
    if ($coupon === 'FIRST15') {
        ...
        if ((int)$cnt->fetchColumn() > 0) {
            echo json_encode(['ok'=>false,'error'=>'Mã FIRST15 chỉ áp dụng cho đơn hàng đầu tiên']);
            exit;
        }
    }
    $tongTien   = (int)round($subtotal * (100 - $VALID_COUPONS[$coupon]) / 100);
    $couponCode = $coupon;
}
...
try {
    ...
    echo json_encode(['ok'=>false,'error'=>'Không tạo được đơn hàng: '.$e->getMessage()]);
}
