...
<link rel="stylesheet" href="assets/css/checkout-buynow.css">
<script src="assets/js/checkout-buynow.js"></script>
      ...
      <div class="checkout-box">
        ...
        <div class="pay-option selected" data-method="VNPay" onclick="selectPayment(this)"
          ...
          <div>
            ...
            <div style="font-size:12px;opacity:.6;margin-top:2px">ATM · Visa · QR Code</div>
          </div>
          <span style="margin-left:auto;background:var(--teal-700);color:#fff;font-size:10px;font-weight:800;padding:3px 8px;border-radius:5px">Khuyến nghị</span>
        </div>
        <div class="pay-option" data-method="Chuyển khoản ngân hàng" onclick="selectPayment(this)">
          <input type="radio" name="pay" value="Chuyen_khoan">
          <div>
            <div style="font-weight:700;font-size:14px">🏦 Chuyển khoản</div>
            <div style="font-size:12px;opacity:.6;margin-top:2px">Quét QR sau khi đặt hàng</div>
          </div>
        </div>
      </div>
    </div>
    ...
    <div class="checkout-box checkout-summary-sticky">
      ...
      <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:16px;padding:14px;background:var(--bg-alt);border-radius:10px">
        ...
        <div style="display:flex;align-items:center;gap:0">
          ...
          <input type="number" id="qtyDisplay" min="1" step="1" value="1"
              onchange="setQty(this.value)" onclick="this.select()" style="
              width:48px;height:34px;border-top:1.5px solid var(--border);
              border-bottom:1.5px solid var(--border);border-left:none;border-right:none;
      ...
      <button class="btn-checkout" onclick="handleCheckout()" style="margin-top:14px">
        Đặt hàng ngay →
      </button>
...
<!-- LOADING MODAL (chuyển đến VNPay) -->
...
<div id="qrModal">
  <div class="qr-box">
    ...
    <button onclick="closeQR()" class="qr-back-btn">
      Quay lại
    </button>
  </div>
</div>
